...
async def get_recent_matches(db: AsyncSession = Depends(database.get_db)):
    ...
    for r in results:
        ...
        team1 = await db.get(models.Team, match_info.team1_id)
        team2 = await db.get(models.Team, match_info.team2_id)
        winner = await db.get(models.Team, r.winner_id)
        tournament = await db.get(models.Tournament, match_info.tournament_id)
        game = await db.get(models.Game, tournament.game_id)
